...
public class PolymorphNeoForgeNetwork {
  ...
  public static <M> void registerC2S(Class<M> clazz, BiConsumer<M, FriendlyByteBuf> encoder,
    ...
    instance.registerMessage(id++, clazz, encoder::accept, decoder::apply, (m, context) -> {
      context.enqueueWork(() -> {
        ServerPlayer sender = context.getSender();

        if (sender != null) {
          handler.accept(m, sender);
        }
      });
  ...
  public static <M> void registerS2C(Class<M> clazz, BiConsumer<M, FriendlyByteBuf> encoder,
    ...
    instance.registerMessage(id++, clazz, encoder::accept, decoder::apply, (m, context) -> {
      context.enqueueWork(
          () -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> handler.accept(m)));
      context.setPacketHandled(true);
    });
  }
}
